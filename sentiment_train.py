...
class DataSetChecker:
  ...
  #το παρακάτω δεν αποτελεί σχόλιο αλλά κείμενο πειγραφής, που εξηγεί ακριβώς τι κάνει κάθε κλάση
    ...
    def run_all_checks(self):
      ...
      return df
    # Ο συγκεκριμένος κώδικας τρέχει μόνο μόνο αν τρέξω αυτό το αρχείο απευθείας σαν πρόγραμμα όχι αν το κάνω import
    #Συνθήκη if, όταν τρέχω αυτό το αρχείο πρώτα θέλω να ελέγχεται το dataset  μου και αν όλα είναι εντάξει τότε θα συνεχίσω με την εκπαίδευση
    #Παρακάτω δημιουργώ ένα αντικείμενο της κλάσης DataSetCheckerκαι του δίνω το όνομα του CSV που θέλω να ελέγξω.
    checker = DataSetChecker(csv_path="sentences.csv")
    #Στη συνέχεια καλώ τη μέθοδο που τρέχει συνολικά τους ελέγχους.
    all_good = checker.run_all_checkers()
    # Αν το αποτέλεσμα είναι True,τότε είμαστε εντάξει
    if all_good:
        print("Τελικό μήνυμα: Το αρχείο sentences.csv είναι έτοιμο να εκπαιδευτεί.")
    else:
        # Αν είναι False, ενδεχομένως κάπου βρέθηκε λάθος 
        print("Τελικό μήνυμα: Το αρχείο sentences.csv έχει κάποια προβλήματα.")
        print("ΔΙΟΡΘΩΣΕ ΤΑ ΛΑΘΗ!")
